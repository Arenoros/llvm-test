...
/***************************/
/* enumvalue_symtable is filled in by enum_declaration_check_c, during stage3 semantic verification, with a list of all
 * enumerated constants declared inside this POU */
...
/*  This is currently needed because generate_c stage 4 needs to handle the array datatype identifiers differently to
 ...
 * elementary_string_type_name <- STRING) (shift: string_type_declaration_size <-  empty ) string_type_declaration_init:
 * ASSIGN character_string (shift: character_string <- "hello!") (reduce: string_type_declaration_init <- ASSIGN
 * character_string) (reduce: string_type_declaration <- identifier ':' elementary_string_type_name
 * string_type_declaration_size string_type_declaration_init ) (reduce: type_declaration <- string_type_declaration)
 *
 *
 ...
 * enumerated constants declared inside this POU */
SYM_REF3(
    function_block_declaration_c, fblock_name, var_declarations, fblock_body, enumvalue_symtable_t enumvalue_symtable;)
 ...
 * instructions that may be executed imedaitely before/after this instruction. In case of an il instruction preceded by
 * a label, the previous_il_instruction will include all IL instructions that jump to this label! It is filled in by the
 * flow_control_analysis_c during stage 3. This will essentially be a doubly linked list of il_instruction_c and
 * il_simple_instruction_c objects!!
 */
 ...
 *   In order to simplify processing of the IL code, stage2 will prepend an artifical (and equivalent) 'LD <il_operand>'
 * IL instruction into the simple_instr_list The il_expression_c.il_operand is maintained, in case we really need to
 * handle it as a special case! See the comments in iec_bison.yy for details and an example.
 */
 ...
 */
/* NOTE: The parameter 'called_fb_declaration'is used to pass data between stage 3 and stage4 (although currently it is
 * not used in stage 4 */
...
/* NOTE: The parameter 'called_fb_declaration' is used to pass data between stage 3 and stage4 (although currently it is
 * not used in stage 4 */
/* NOTE: The parameter 'deprecated_operation' indicates that the operation, with the specific data types being used, is
 * currently defined in the standard as being deprecated. This variable is filled in with the correct value in stage 3
 * (narrow_candidate_datatypes_c) and currently only used in stage 3 (print_datatypes_error_c).
 */
...
SYM_REF1(deref_expression_c, exp) /* an extension to the IEC 61131-3 standard - based on the IEC 61131-3 v3 standard.
                                     DREF() -> dereferences an address!        */
