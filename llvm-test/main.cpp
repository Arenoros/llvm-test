...
namespace {
    cl::opt<std::string> InputIR("input-IR",
                                 cl::desc("Specify the name of an IR file to load for function definitions"),
                                 cl::value_desc("input IR file name"));

    cl::opt<bool> VerboseOutput("verbose",
                                cl::desc("Enable verbose output (results, IR, etc.) to stderr"),
                                cl::init(false));

    cl::opt<bool> SuppressPrompts("suppress-prompts", cl::desc("Disable printing the 'ready' prompt"), cl::init(false));

    cl::opt<bool> DumpModulesOnExit("dump-modules",
                                    cl::desc("Dump IR from modules to stderr on shutdown"),
                                    cl::init(false));

    cl::opt<bool> EnableLazyCompilation("enable-lazy-compilation",
...
ExprAST* ErrorRet(const char* Str) {
...
static ExprAST* ParseIdentifierExpr() {
    ...
    if (CurTok != ')') {
        while (1) {
            ...
            if (CurTok != ',')
                return ErrorRet("Expected ')' or ',' in argument list");
            getNextToken();
        }
    }
...
static ExprAST* ParseForExpr() {
    ...
    if (CurTok != tok_identifier)
        return ErrorRet("expected identifier after for");

    std::string IdName = IdentifierStr;
    getNextToken();  // eat identifier.

    if (CurTok != '=')
        return ErrorRet("expected '=' after for");
    getNextToken();  // eat '='.
    ...
    if (Start == 0)
        return 0;
    if (CurTok != ',')
        return ErrorRet("expected ',' after for start value");
    getNextToken();
    ...
    if (CurTok != tok_in)
        return ErrorRet("expected 'in' after for");
    getNextToken();  // eat 'in'.
...
static ExprAST* ParseVarExpr() {
    ...
    if (CurTok != tok_identifier)
        return ErrorRet("expected identifier after var");
    ...
    while (1) {
        ...
        if (CurTok != tok_identifier)
            return ErrorRet("expected identifier list after var");
    }

    // At this point, we have to have 'in'.
    if (CurTok != tok_in)
        return ErrorRet("expected 'in' keyword after 'var'");
    getNextToken();  // eat 'in'.
...
static ExprAST* ParsePrimary() {
    switch (CurTok) {
    default:
        return ErrorRet("unknown token when expecting an expression");
    case tok_identifier:
...
Function* MCJITHelper::getFunction(const std::string FnName) {
    ...
    for (it = begin; it != end; ++it) {
        ...
        if (F) {
            ...
            if (!PF)
                PF = Function::Create(F->getFunctionType(), Function::ExternalLinkage, FnName, CurrentModule);
            return PF;
        }
    }
    return NULL;
...
ExecutionEngine* MCJITHelper::compileModule(Module* M) {
    ...
    ptr.
    ExecutionEngine* EE =
        EngineBuilder(std::make_unique(M)).setErrorStr(&ErrStr).setMCJITMemoryManager(std::make_unique<HelpingMemoryManager>(this)).create();
    if (!EE) {
